...
def softmax_loss(x, y):
    ...
    - y: Vector of labels, of shape (N,) where y[i] is 
	the label (categorical response) for x[i] and 0 <= y[i] < C
    ...
    loss = np.sum(-np.log(pr[range(N), y]))/N # why /N
    dx = np.copy(pr)
    dx[range(N),y] -= 1
    dx /= N
    ###########################################################################
